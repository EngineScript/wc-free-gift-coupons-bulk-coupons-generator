 ...
 * Text Domain: wc-free-gift-bulk-coupons-generator
...
class WooCommerceFreeGiftBulkCoupons {
    ...
    public function init() {
        ...
        load_plugin_textdomain('wc-free-gift-bulk-coupons-generator', false, dirname(plugin_basename(__FILE__)) . '/languages');
    ...
    public function woocommerce_missing_notice() {
        ...
        $message = sprintf(
            esc_html__('WC Free Gift Coupons Bulk Coupon Generator requires WooCommerce to be installed and active. You can download %s here.', 'wc-free-gift-bulk-coupons-generator'),
    ...
    public function add_admin_menu() {
        add_submenu_page(
            'woocommerce',
            __('Free Gift Bulk Coupons', 'wc-free-gift-bulk-coupons-generator'),
            __('Coupon Generator', 'wc-free-gift-bulk-coupons-generator'),
            'manage_woocommerce',
    ...
    private function handle_coupon_generation() {
        ...
        if (!isset($_POST['scg_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['scg_nonce'])), 'scg_generate_coupons_action')) {
            wp_die(esc_html__('Security check failed. Please try again.', 'wc-free-gift-bulk-coupons-generator'));
        }

        // Verify user capabilities
        if (!current_user_can('manage_woocommerce')) {
            wp_die(esc_html__('You do not have permission to generate coupons.', 'wc-free-gift-bulk-coupons-generator'));
        }
        ...
        if (get_transient($transient_key)) {
            add_action('admin_notices', function() {
                echo '<div class="notice notice-error is-dismissible"><p>' . 
                     esc_html__('Coupon generation already in progress. Please wait before starting another batch.', 'wc-free-gift-bulk-coupons-generator') . 
                     '</p></div>';
            });
            return;
        }
        ...
        if (empty($product_ids) || empty($number_of_coupons)) {
            add_action('admin_notices', function() {
                echo '<div class="notice notice-error is-dismissible"><p>' . 
                     esc_html__('Please select at least one product and specify the number of coupons to generate.', 'wc-free-gift-bulk-coupons-generator') . 
                     '</p></div>';
            });
            return;
        }
        ...
        foreach ($product_ids as $product_id) {
            if ($product_id <= 0 || $product_id > PHP_INT_MAX) {
                add_action('admin_notices', function() {
                    echo '<div class="notice notice-error is-dismissible"><p>' . 
                         esc_html__('Invalid product selection. Please try again.', 'wc-free-gift-bulk-coupons-generator') . 
                         '</p></div>';
                });
                return;
            }
        ...
        if ($number_of_coupons <= 0 || $number_of_coupons > 100) {
            add_action('admin_notices', function() {
                echo '<div class="notice notice-error is-dismissible"><p>' . 
                     esc_html__('Maximum number of coupons that can be generated at once is 100.', 'wc-free-gift-bulk-coupons-generator') . 
                     '</p></div>';
            });
            return;
        }
        ...
        if ($generated_coupons > 0) {
            add_action('admin_notices', function() use ($generated_coupons) {
                echo '<div class="notice notice-success is-dismissible"><p>' . 
                     ...
                     sprintf(esc_html__('Successfully generated %d coupons.', 'wc-free-gift-bulk-coupons-generator'), esc_html($generated_coupons)) . 
                     '</p></div>';
            });
        } else {
            add_action('admin_notices', function() {
                echo '<div class="notice notice-error is-dismissible"><p>' . 
                     esc_html__('Failed to generate coupons. Please try again.', 'wc-free-gift-bulk-coupons-generator') . 
                     '</p></div>';
            });
        }
    }
        ...
        for ($i = 1; $i <= $number_of_coupons; $i++) {
            ...
            try {
                ...
                $coupon->set_description(sprintf(
                    __('Auto-generated coupon for %1$s (Batch %2$d/%3$d)', 'wc-free-gift-bulk-coupons-generator'),
                    $products_text,
                    $i,
                    $number_of_coupons
                ));
                ...
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    ...
                    wc_get_logger()->error(sprintf(__('SCG Error generating coupon: %s', 'wc-free-gift-bulk-coupons-generator'), $e->getCode()), array('source' => 'wc-free-gift-bulk-coupons-generator'));
                }
                $i--; // Try again with same counter
                continue;
            }
    ...
    public function admin_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('WC Free Gift Coupons Bulk Coupon Generator', 'wc-free-gift-bulk-coupons-generator'); ?></h1>
            <p><?php esc_html_e('Generate bulk free gift coupons that work with the Free Gift Coupons for WooCommerce plugin. These coupons are created with the proper data structure required for free gift functionality.', 'wc-free-gift-bulk-coupons-generator'); ?></p>
            ...
            <div class="scg-admin-container">
                <div class="scg-main-content">
                    <form method="post" action="" class="scg-form">
                        ...
                        <table class="form-table">
                            <tr>
                                <th scope="row">
                                    <label for="product_id"><?php esc_html_e('Select Products', 'wc-free-gift-bulk-coupons-generator'); ?></label>
                                </th>
                                <td>
                                    ...
                                    <p class="description">
                                        <?php esc_html_e('Select one or more products that will be given as free gifts with the coupon. Hold Ctrl (Windows) or Cmd (Mac) to select multiple products.', 'wc-free-gift-bulk-coupons-generator'); ?>
                                    </p>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <th scope="row">
                                    <label for="number_of_coupons"><?php esc_html_e('Number of Coupons', 'wc-free-gift-bulk-coupons-generator'); ?></label>
                                </th>
                                <td>
                                    <input type="number" name="number_of_coupons" id="number_of_coupons" 
                                           class="regular-text" min="1" max="100" value="10" required>
                                    <p class="description">
                                        <?php esc_html_e('Enter the number of coupons to generate (maximum 100).', 'wc-free-gift-bulk-coupons-generator'); ?>
                                    </p>
                                    <div class="scg-warning-box">
                                        <p class="scg-warning-text">
                                            <span class="dashicons dashicons-warning" style="color: #d63638;"></span>
                                            <?php esc_html_e('Note: Coupon generation can be time-consuming. Generating large numbers of coupons may cause the page to timeout based on your server\'s PHP timeout settings. If you need to generate many coupons, consider doing it in smaller batches.', 'wc-free-gift-bulk-coupons-generator'); ?>
                                        </p>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <th scope="row">
                                    <label for="coupon_prefix"><?php esc_html_e('Coupon Prefix', 'wc-free-gift-bulk-coupons-generator'); ?></label>
                                </th>
                                <td>
                                    <input type="text" name="coupon_prefix" id="coupon_prefix" 
                                           class="regular-text" maxlength="10" placeholder="e.g. GIFT">
                                    <p class="description">
                                        <?php esc_html_e('Optional prefix for coupon codes (e.g. GIFT-ABC123DEF456).', 'wc-free-gift-bulk-coupons-generator'); ?>
                                    </p>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <th scope="row">
                                    <label for="discount_type"><?php esc_html_e('Discount Type', 'wc-free-gift-bulk-coupons-generator'); ?></label>
                                </th>
                                <td>
                                    <select name="discount_type" id="discount_type" class="regular-text">
                                        <option value="free_gift"><?php esc_html_e('Free Gift', 'wc-free-gift-bulk-coupons-generator'); ?></option>
                                        <option value="percent"><?php esc_html_e('Percentage Discount', 'wc-free-gift-bulk-coupons-generator'); ?></option>
                                        <option value="fixed_cart"><?php esc_html_e('Fixed Cart Discount', 'wc-free-gift-bulk-coupons-generator'); ?></option>
                                        <option value="fixed_product"><?php esc_html_e('Fixed Product Discount', 'wc-free-gift-bulk-coupons-generator'); ?></option>
                                    </select>
                                    <p class="description">
                                        <?php esc_html_e('Select the type of discount for the coupons.', 'wc-free-gift-bulk-coupons-generator'); ?>
                                    </p>
                                </td>
                            </tr>
                        </table>
                        ...
                        <p class="submit">
                            <input type="submit" name="scg_generate_coupons" class="button-primary" 
                                   value="<?php esc_attr_e('Generate Free Gift Coupons', 'wc-free-gift-bulk-coupons-generator'); ?>">
                        </p>
                    </form>
                </div>
                ...
                <div class="scg-sidebar">
                    <div class="scg-info-box">
                        <h3><?php esc_html_e('Information', 'wc-free-gift-bulk-coupons-generator'); ?></h3>
                        <ul>
                            <li><?php esc_html_e('Maximum 100 coupons can be generated at once', 'wc-free-gift-bulk-coupons-generator'); ?></li>
                            <li><?php esc_html_e('Coupons are set to expire after 1 year', 'wc-free-gift-bulk-coupons-generator'); ?></li>
                            <li><?php esc_html_e('Each coupon can only be used once', 'wc-free-gift-bulk-coupons-generator'); ?></li>
                            <li><?php esc_html_e('Coupons are set for individual use only', 'wc-free-gift-bulk-coupons-generator'); ?></li>
                            <li><?php esc_html_e('Generated coupons appear in WooCommerce > Coupons', 'wc-free-gift-bulk-coupons-generator'); ?></li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <div class="scg-footer">
                <p class="scg-repo-link">
                    <a href="https://github.com/EngineScript/WC-Free-Gift-Coupons-Bulk-Coupons-Generator" target="_blank" rel="noopener noreferrer">
                        <?php esc_html_e('View on GitHub', 'wc-free-gift-bulk-coupons-generator'); ?>
                    </a>
                    |
                    <a href="https://github.com/EngineScript/WC-Free-Gift-Coupons-Bulk-Coupons-Generator/issues" target="_blank" rel="noopener noreferrer">
                        <?php esc_html_e('Report Issues', 'wc-free-gift-bulk-coupons-generator'); ?>
                    </a>
                </p>
            </div>
        </div>
